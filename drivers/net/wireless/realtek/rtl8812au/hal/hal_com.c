...
void r8812au_sec_write_cam(_adapter *adapter, u8 addr, u32 wdata)
{
	_mutex *mutex = &adapter_to_dvobj(adapter)->cam_ctl.sec_cam_access_mutex;
	u32 cnt = 0;
	systime start = 0, end = 0;
...
void rtw_sec_write_cam_ent(_adapter *adapter, u8 id, u16 ctrl, u8 *mac, u8 *key)
{
	...
	for (; j >= 0; j--) {
		...
		r8812au_sec_write_cam(adapter, addr, wdata);
	}

#if defined(CONFIG_RTL8192F)
	r8812au_sec_write_cam(adapter, addr1, wdata1);
#endif
}
...
void rtw_sec_clr_cam_ent(_adapter *adapter, u8 id)
{
	...
	addr = (id << 3);
	r8812au_sec_write_cam(adapter, addr, 0);
}
